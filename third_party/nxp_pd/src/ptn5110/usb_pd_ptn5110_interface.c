...
pd_status_t PDPTN5110_Init(pd_handle pdHandle)
{
    ...
    PD_I2cReleaseBus releaseBus = phyConfig->i2cReleaseBus;

    alertConfig.direction = kHAL_GpioDirectionIn;
    alertConfig.port      = phyConfig->alertPort;
    alertConfig.pin       = phyConfig->alertPin;
    alertConfig.level     = 1;
    alertConfig.pullMode  = kHAL_GpioNoPull;
    alertConfig.intrFlags = 0;

#if defined(PD_CONFIG_TARGET_ESP32S3) && (PD_CONFIG_TARGET_ESP32S3)
    pd_phy_esp32s3_config_t *platformConfig = (pd_phy_esp32s3_config_t *)phyConfig;
    pd_i2c_esp32_config_t i2cConfig         = {
        .sdaPin             = platformConfig->sdaPin,
        .sclPin             = platformConfig->sclPin,
        .busSpeed_Hz        = platformConfig->i2cBusSpeed_Hz,
        .enableInternalPullup = platformConfig->enableInternalPullup,
    };

    if (PD_I2cConfigureEsp32((uint8_t)phyConfig->i2cInstance, &i2cConfig, &releaseBus) != kStatus_PD_Success)
    {
        return kStatus_PD_Error;
    }

    alertConfig.pullMode  = platformConfig->alertPullMode;
    alertConfig.intrFlags = platformConfig->alertIsrFlags;
#elif defined(PD_CONFIG_TARGET_ARDUINO) && (PD_CONFIG_TARGET_ARDUINO)
    pd_phy_arduino_config_t *platformConfig = (pd_phy_arduino_config_t *)phyConfig;
    pd_i2c_arduino_config_t i2cConfig       = {
        .wire                = platformConfig->wire,
        .sdaPin              = platformConfig->sdaPin,
        .sclPin              = platformConfig->sclPin,
        .busSpeed_Hz         = platformConfig->i2cBusSpeed_Hz,
        .enableInternalPullup = platformConfig->enableInternalPullup,
    ...
    if (PD_I2cConfigureArduino((uint8_t)phyConfig->i2cInstance, &i2cConfig, &releaseBus) != kStatus_PD_Success)
    {
        return kStatus_PD_Error;
    }
    ...
    pd_phy_stm32_config_t *platformConfig = (pd_phy_stm32_config_t *)phyConfig;
    pd_i2c_stm32_config_t i2cConfig       = {
        .hi2c      = platformConfig->hi2c,
        .timeoutMs = platformConfig->i2cTimeoutMs,
        .sclPort   = platformConfig->sclPort,
        .sclPin    = platformConfig->sclPin,
        .sdaPort   = platformConfig->sdaPort,
        .sdaPin    = platformConfig->sdaPin,
    };

    if (PD_I2cConfigureStm32((uint8_t)phyConfig->i2cInstance, &i2cConfig, &releaseBus) != kStatus_PD_Success)
    {
        return kStatus_PD_Error;
    }
    ...
    alertConfig.pin       = 0;
    alertConfig.pullMode  = platformConfig->alertPullMode;
    alertConfig.intrFlags = (uint32_t)(uintptr_t)platformConfig;
#endif

    phyConfig->i2cReleaseBus = releaseBus;

    /* I2C initialization */
    if (PD_I2cInit(&pdInstance->i2cHandle,
                   (uint8_t)(phyConfig->i2cInstance),
                   phyConfig->i2cSrcClock,
                   releaseBus) != kStatus_PD_Success)
    {
        return kStatus_PD_Error;
    }
